  ...
  describe Spree::OrderMailer, type: :mailer do
    ...
    let(:line_item) { build(:line_item, variant: variant, order: order, quantity: 1, price: 4.99) }
    let(:variant)   { stub_model(Spree::Variant, product: product) }
    let(:product)   { stub_model(Spree::Product, name: %|The "BEST" product|, product_type: type) }
    let(:type)      { build_stubbed(:product_type) }
    let(:price)     { stub_model(Spree::Price, variant: variant, amount: 5.00) }
    ...
    it "confirm_email accepts an order id as an alternative to an Order object" do
      ...
      expect { Spree::OrderMailer.confirm_email(order.id) }.not_to raise_error
    end

    it "cancel_email accepts an order id as an alternative to an Order object" do
      expect(Spree::Order).to receive(:find).with(order.id).and_return(order)
      expect{ Spree::OrderMailer.cancel_email(order.id) }.not_to raise_error
    end
    ...
    context "emails must be translatable" do
      let(:pt_br_cancel_mail) do
        { spree:
          { order_mailer:
            { cancel_email:
              { order_summary_canceled: "Resumo da Pedido [CANCELADA]" }
            }
          }
        }
      end

      let(:pt_br_confirm_mail) do
        { spree: { order_mailer: { confirm_email: { dear_customer: "Caro Cliente," } } } }
      end
    ...
    context "when digital download present" do
      let(:digital_message) do
        "Your PDF pattern will be sent in a separate email, it should be with you in " +
          "5-10 minutes from receiving this order confirmation."
      end
      ...
      it "contains a message" do
        ...
        expect(header["digital_message"]).to eq digital_message
      end
    end
