...
describe Spree::GiftCard do
  ...
  context "class methods" do
    ...
    it "matches code of gift card format" do
      expect(subject.match_gift_card_format?('D1C7-D614B8-511C')).to be_true
    end
    it "doesn't matches other codes" do
      expect(subject.match_gift_card_format?('RACHELRUTT10')).to be_false
    end
  end
  ...
  context "#eligible?" do
    ...
    it "checks expiry_date" do
      ...
      expect(card.eligible?(order)).to be_false
    end
    ...
    it "checks beneficiary order has not changed  when state is redeemed" do
      ...
      expect(card.eligible?(second_order)).to be_false
    end
    ...
    it "returns true for state redeemed and same order" do
      card = build(:gift_card, expiry_date: 1.day.from_now, beneficiary_order: order, beneficiary_email: order.email, state: 'redeemed')
      expect(card.eligible?(order)).to be_true
    end
  ...
  context 'states' do
    ...
    context 'changes' do
      it 'tells if can change to desired state from not_redeemed' do
        ...
        expect(card.change_state_to?('redeemed')).to be_true
        expect(card.change_state_to?('paused')).to be_true
        expect(card.change_state_to?('cancelled')).to be_true
        expect(card.change_state_to?('refunded')).to be_true
      end

      it 'tells if can change to desired state from redeemed' do
        card = build(:gift_card, state: 'redeemed')

        expect(card.change_state_to?('not_redeemed')).to be_false
        expect(card.change_state_to?('paused')).to be_false
        expect(card.change_state_to?('cancelled')).to be_false
        expect(card.change_state_to?('refunded')).to be_true
      end
      ...
      it 'tells if can change to desired state from paused' do
        ...
        expect(card.change_state_to?('not_redeemed')).to be_true
        expect(card.change_state_to?('redeemed')).to be_false
        expect(card.change_state_to?('cancelled')).to be_true
        expect(card.change_state_to?('refunded')).to be_true
      end
      ...
      it 'tells if can change to desired state from cancelled' do
        ...
        expect(card.change_state_to?('redeemed')).to be_false
        expect(card.change_state_to?('paused')).to be_false
        expect(card.change_state_to?('refunded')).to be_false
      end
