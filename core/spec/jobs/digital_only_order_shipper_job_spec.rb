...
describe Spree::DigitalOnlyOrderShipperJob do
  ...
  let(:digital) { create(:product_type, is_digital: true)}
  let(:non_digital) { create(:product_type, is_digital: false)}

  let(:digital_product) { create(:product, product_type: digital)}
  let(:physical_product) { create(:product, product_type: non_digital)}
