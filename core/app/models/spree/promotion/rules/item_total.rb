        ...
        def eligible?(order, options = {})
          ...
          hash.each do |zone_id, currency_amount_enabled|
            ...
            currency_amount_enabled.each do |currency, amount_enabled|
              next unless order.currency == currency && amount_enabled["enabled"] == "true"
              order_total = order.item_total
              next unless order_total.send(:>=, BigDecimal.new(amount_enabled["amount"].to_s))

                  # If everything else is good but address as not been defined
                  # then return true
                  # TODO: this should probably choose the cheapest zone rather
                  # than the first
