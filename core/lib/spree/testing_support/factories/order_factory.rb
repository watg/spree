    ...
    factory :order_with_line_items do
      ...
      trait :with_marketing_type do
        after(:create) do |order, _evaluator|
          order.line_items.first.variant.product.update_column(
            :marketing_type_id, create(:marketing_type).id)
        end
      end
      ...
      trait :with_product_group do
        after(:create) do |order, _evaluator|
          order.line_items.first.variant.product.update_column(
            :product_group_id, create(:product_group).id)
        end
      end

      after(:create) do |order, evaluator|
        order.line_items = create_list(
          :line_item, evaluator.line_items_count, order: order, price: evaluator.line_items_price)
