...
describe Spree::PinterestService do
  ...
  it "returns a correct OpenStruct response with a product" do
    ...
    variant_permalink = variant.option_values.first.name
    variant.in_stock_cache = true
    variant.save
    url = "http://www.woolandthegang.com/shop/products/#{product.permalink}/#{variant_permalink}"
    
    outcome = Spree::PinterestService.run({url: url})
    
    outcome.success?.should == true
    outcome.result.product_id.should == product.permalink
  ...
  it "returns a could_not_find_product error" do
    url = "http://www.woolandthegang.com/shop/products/invalid_product/variant_permalink"
    outcome = Spree::PinterestService.run({url: url})
