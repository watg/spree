...
describe Api::Dashboard::Office::FormatTodayOrdersByHour, type: :interaction do
  let!(:orders_early) { create_list(:order, 4) }
  let!(:orders_recent) { create_list(:order, 4) }
  subject { described_class.new }
  describe "execute" do
    before do
      new_time = Time.local(2015, 9, 1, 12, 0, 0)
      Timecop.freeze(new_time)
      orders_early.each { |o| o.update_attributes(completed_at: Time.zone.now.at_beginning_of_day) }
      orders_recent.each { |o| o.update_attributes(completed_at: 1.hour.ago) }
    end
