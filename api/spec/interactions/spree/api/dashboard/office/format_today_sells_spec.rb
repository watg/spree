        ...
        describe FormatTodaySells, type: :interaction do
          ...
          describe "execute" do
            it "returns todays orders by currency" do
              allow_any_instance_of(Spree::Api::Dashboard::Office::FindTodayValidOrders).to receive(:run).and_return([usd_order, eur_order, gbp_order])
              expect(subject.run).to eq("EUR" => BigDecimal.new(11), "GBP" => BigDecimal.new(12), "USD" => BigDecimal.new(10))
            end
          end
        end
      end
