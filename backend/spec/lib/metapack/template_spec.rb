...
describe "Erb Template" do
  describe :create_and_allocate_consignment_with_booking_code do
    ...
    describe "#xml" do
      subject { super().xml }
      it { is_expected.to eq(xml_fixture("create_and_allocate_consignments_with_booking_code.xml")) }
    end
  end

  describe :create_labels_as_pdf do
    subject { Metapack::SoapTemplate.new(:create_labels_as_pdf, consignment_code: 12_345) }

    describe "#xml" do
      subject { super().xml }
      it { is_expected.to eq(xml_fixture("create_labels_as_pdf.xml")) }
    end
  end
