...
describe Spree::ShippingManifestService::UniqueProducts do
  ...
  context "Assembly" do
    ...
    context "with assembled parts and a main part" do
      let(:part2_variant) { create(:base_variant) }
      let(:part3_variant) { create(:base_variant) }
      let!(:part2) { create(:line_item_part, line_item: line_item_1, quantity: 2, price: 24.00, variant: part2_variant, assembled: true, main_part: true) }
      let!(:part3) { create(:line_item_part, line_item: line_item_1, quantity: 2, price: 5.00, variant: part3_variant, assembled: true) }
      ...
      it "uses the main part, ignores the assembled parts" do
        unique_products = subject.result
        ...
        expect(up2[:mid_code]).to eq supplier_2.mid_code
        expect(up2[:quantity]).to eq 4
        expect(up2[:total_price].to_f).to eq 75.00
        expect(up2[:group]).to eq part2_variant.product.product_group
