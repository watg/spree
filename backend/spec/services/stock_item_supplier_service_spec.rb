...
describe Spree::StockItemSupplierService do
  subject { described_class.run(variant: variant, supplier_id: supplier_id) }
  ...
  describe "::run" do
    context "requires supplier" do
      before { allow_any_instance_of(described_class).to receive_messages requires_supplier: true }
    ...
    context "does not require supplier" do
      before { allow_any_instance_of(described_class).to receive_messages requires_supplier: false }
