  ...
  module Admin
    module BaseHelper
      ...
      def order_to_be_shipped
        ...
        order_set = Spree::Order.includes(:shipments).includes(:payments).where('spree_orders.state' => :complete, 'spree_payments.state' => :completed, 'spree_shipments.state' => :ready).map(&:line_items).flatten.map {|e| [(e.variant.sku rescue nil), e.quantity]}
