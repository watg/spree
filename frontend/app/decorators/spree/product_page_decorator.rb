class Spree::ProductPageDecorator < Draper::Decorator
  ...
  def kit_description
    description = object.knit_your_own.product.description_for(object.target).to_s rescue Spree.t(:product_has_no_description)
    description = description.gsub(/(.*?)\r?\n\r?\n/m, '\1<br><br>')
    description.gsub(/_/, '').gsub(/-/, '&ndash;').html_safe
  end

  def kit_name
    object.knit_your_own.product.name
  end
